...
namespace AirportServer.Domain.Models
{
    public class Airport
    {
        ...
        public Airport(IAirportRepository airportRepository)
        {
        ...
        private void TryEnterToStation(Station stat, Airplane newAirplane)
        {
            var station = stat;
            lock(station.Locker)
            {
                ...
                nextStation.AddtoWaitingList(nextAirplane);
            }
        }

        private void ExitPrevStationIfExists(Airplane nextAirplane)
        {
            if (nextAirplane.CurrentVisit == null)//if its is first station
                return;
            Visit prevVisit;
